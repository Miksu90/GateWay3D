...
bool collideWithMap(const glm::vec3& position, const Map& map, float radius) {
    ...
    // Debug print for exact position and grid cell
    std::cout << "Position: ("
              << position.x << ", "
              << position.z << ") "
              << "Grid Cell: ("
              ...
              << gridZ << ") "
              << "Grid Value: "
              << (gridX >= 0 && gridX < map.width && gridZ >= 0 && gridZ < map.height
                  ? std::to_string(map.grid[gridZ][gridX])
                  : "OUT OF BOUNDS")
              << std::endl;

    // Extremely conservative safety radius
    float safetyRadius = radius * 0.90f;
    ...
    // Extremely precise boundary prevention
    if (position.x - safetyRadius < cellMaxX && position.x + safetyRadius > cellMinX &&
        position.z - safetyRadius < cellMaxZ && position.z + safetyRadius > cellMinZ) {
        // Check if we're actually inside a wall cell
        if (gridX >= 0 && gridX < map.width && gridZ >= 0 && gridZ < map.height) {
            if (map.grid[gridZ][gridX] == 1) {
                std::cout << "COLLISION DETECTED IN GRID CELL" << std::endl;
                return true;
            }
        }
    }

    // Rest of the previous collision detection logic...
    return false;
}
...
void processMovement(Camera& camera, const Map& map, float deltaTime) {
    ...
    const int NUM_STEPS = 30; // Increased number of steps for more precision
